...
export function useGame(
  ...
  return {
    ...
    maxGames: state.max_games ?? 10,
    passPolicy: state.pass_policy,
    timerEnabled: state.timer_enabled,
    completionPolicy: state.completion_policy,
    player,
    playerPosition,
    playerHand: hand,
    leftPlayer,
    ...
    declarerScore:
      state.declarer === null ? 0 : state.player_scores[state.declarer],
    opponentScore:
      state.declarer === null
        ? 0
        : state.player_scores.reduce(
            (sum, score, index) =>
              index === state.declarer ? sum : sum + score,
            0,
          ),
