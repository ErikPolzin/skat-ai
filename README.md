...
- Configurable AI players backed by database agent configs. Current card-play options include heuristic, perfect-information minimax variants, neural inference, and random play.
...
When using the SQL schemas, the seeded agents include heuristic and neural card-play variants:
...
Supported card-play strategy names in code include `heuristic`, `minimax`, `minimax-heuristic`, `neural`, and `random`.
...
go run ./cmd/eval_agent -agent-type minimax -games 500
